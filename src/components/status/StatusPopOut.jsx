	...
	return (
		<div>
			<Carousel showArrows={true} autoPlay interval="2000" transitionTime="1000" showThumbs={false} swipeable useKeyboardArrows onChange={onChange} onClickItem={onClickItem} onClickThumb={onClickThumb}>
				{statuses.map(status => (
					<img style={{ marginTop: 10 }} height="500px" width="400px" key={status._id} objectFit="contain" src={`${config.API_URL}post/stream-video?streamFile=${status.file}` || `https://ui-avatars.com/api/?name=${status?.user?.name}`} alt="status" />
